...
var _marked = /*#__PURE__*/regeneratorRuntime.mark(gen3),
    _marked2 = /*#__PURE__*/regeneratorRuntime.mark(gen4);
	...
	ad.addEventListener('click', function () {
		...
		setTimeout(function () {
			if (bgChangeState == 'before') {
				_this.style.background = color;
				_this.innerHTML = '再点我2s还原';
				bgChangeState = 'after';
			} else {
				_this.style.background = 'green';
				_this.innerHTML = '点我2s变色';
...
//es5
var kzDemoArrayResult = kzDemoArray1.concat(kzDemoArray2);
console.log(kzDemoArrayResult);
//es6
var kzDemoArrayResultEs6 = [].concat(kzDemoArray1, kzDemoArray2);
console.log(kzDemoArrayResultEs6);

//数组克隆(浅拷贝: 会拷贝对象的引用地址，如果对象属性值发生改变则拷贝的地方的值也会发生改变)
//修改值不会改变，原始数组重新赋值也不会改变
var kzDemoArray3 = ['z', 's', 'm', 'l'];
var kzDemoArray4 = [].concat(kzDemoArray3);
kzDemoArray4[0] = 'a';
console.log(kzDemoArray3);
console.log(kzDemoArray4);

//修改对象的属性值会一起改变
var kzDemoArray5 = [{ name: '张三', age: 12 }, { name: '李四', age: 15 }];
var kzDemoArray6 = [].concat(kzDemoArray5);
...
function gen3() {
	return regeneratorRuntime.wrap(function gen3$(_context) {
		while (1) {
...
function gen4() {
	var userInfo, other;
	return regeneratorRuntime.wrap(function gen4$(_context2) {
		while (1) {
 ...
 * 按照需求实现以下功能
 */
var setDemoArr = [1, 3, 5, 6, 8, 1, 5, 7];
//数组去重
var result = new Set(setDemoArr);
//使用扩展运算符展开元素
result = [].concat(_toConsumableArray(result));
console.log(result);

//求数组交集
var setDemoArr2 = [5, 8, 7, 0, 9];
result = [].concat(_toConsumableArray(new Set(setDemoArr))).filter(function (item) {
	return new Set(setDemoArr2).has(item);
});
console.log(result);

//求数组并集
result = [].concat(setDemoArr, setDemoArr2);
